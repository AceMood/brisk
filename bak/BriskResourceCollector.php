...
class BriskResourceCollector {
    ...
    public static function widgetEnd() {
        ...
        if (self::$widgetStaticResource[BriskConfig::JS]) {
            $ret[BriskConfig::JS] = self::$widgetStaticResource[BriskConfig::JS];
        }
        if (self::$widgetStaticResource[BriskConfig::CSS]) {
            $ret[BriskConfig::CSS] = self::$widgetStaticResource[BriskConfig::CSS];
        }
    ...
    public static function getPageStaticResource() {
        ...
        if (self::$pageRequireAsync) {
            self::$pageStaticResource[BriskConfig::ATTR_ASYNCLOADED] =
                self::getAsyncResourceMap(self::$pageRequireAsync);
        }
    ...
    public static function getAsyncResourceMap($arrAsync, $cdn = '') {
        ...
        // js结构
        if (isset($arrAsync[BriskConfig::JS])) {
            foreach ($arrAsync[BriskConfig::JS] as $id => $res) {
                $deps = array();
                $css = array();
                ...
                if (!empty($res[BriskConfig::ATTR_DEP])) {
                    foreach ($res[BriskConfig::ATTR_DEP] as $symbol) {
                        $inPkg = false;
                        // 已加载的资源 需要动态删除
                        if (in_array($symbol, self::$pageStaticResource[BriskConfig::JS])) {
                            continue;
                        }

                        // 若依赖的模块已经在打包中
                        $dep = self::getResource(BriskConfig::JS, $symbol);
                        if (isset($dep[BriskConfig::ATTR_IN])) {
                            $pkgId = $dep[BriskConfig::ATTR_IN][0];
                            if (in_array($pkgId, self::$pageStaticResource[BriskConfig::JS])) {
                                continue;
                            }
                            $inPkg = true;
                        }
                ...
                if (!empty($res[BriskConfig::ATTR_CSS])) {
                    foreach ($res[BriskConfig::ATTR_CSS] as $symbol) {
                        $inPkg = false;
                        // 已加载
                        if (in_array($symbol, self::$pageStaticResource[BriskConfig::CSS])) {
                            continue;
                        }

                        // 若依赖的模块已经在打包中
                        $dep = self::getResource(BriskConfig::CSS, $symbol);
                        if (isset($dep[BriskConfig::ATTR_IN])) {
                            $pkgId = $dep[BriskConfig::ATTR_IN][0];
                            if (in_array($pkgId, self::$pageStaticResource[BriskConfig::CSS])) {
                                continue;
                            }
                            $inPkg = true;
                        }
                ...
                if (!empty($res[BriskConfig::ATTR_IN])) {
                    $arrResourceMap[BriskConfig::JS][$id][BriskConfig::ATTR_IN] = $res[BriskConfig::ATTR_IN];
                    //如果包含到了某一个包，则模块的url是多余的
                    //if (!isset($_GET['__debug'])) {
                    //  unset($arrResourceMap[BriskConfig::JS][$id][BriskConfig::ATTR_URI]);
                    //}
                }

                $arrResourceMap[BriskConfig::JS][$id] = array(
        ...
        // css结构
        if (isset($arrAsync[BriskConfig::CSS])) {
            foreach ($arrAsync[BriskConfig::CSS] as $symbol => $res) {
                $css = array();
                if (!empty($res[BriskConfig::ATTR_CSS])) {
                    foreach ($res[BriskConfig::ATTR_CSS] as $symbol) {
                        $inPkg = false;
                ...
                if (!empty($res[BriskConfig::ATTR_IN])) {
                    $arrResourceMap[BriskConfig::CSS][$symbol][BriskConfig::ATTR_IN] = $res[BriskConfig::ATTR_IN];
                }

                $arrResourceMap[BriskConfig::CSS][$symbol] = array(
                    BriskConfig::ATTR_URI => $cdn . $res[BriskConfig::ATTR_URI],
                    BriskConfig::ATTR_CSS => $css
                );
            }
        ...
        if (isset(self::$loadedResources[$type][$symbol])) {
            ...
            if (isset(self::$map[$namespace]) || self::register($namespace)) {
                ...
                if (isset($res)) {
                    ...
                    //if ($async && $res['type'] === BriskConfig::JS) {
                    if ($async) {
    ...
    private static function loadDependencies($res, $async) {
        if (isset($res[BriskConfig::ATTR_DEP])) {
            foreach ($res[BriskConfig::ATTR_DEP] as $symbol) {
                self::load(BriskConfig::JS, $symbol, $async);
            }
        }

        if (isset($res[BriskConfig::ATTR_CSS])) {
            foreach ($res[BriskConfig::ATTR_CSS] as $symbol) {
                self::load(BriskConfig::CSS, $symbol, $async);
            }
        }
        ...
        if (isset($res[BriskConfig::ATTR_ASYNCLOADED])) {
            foreach ($res[BriskConfig::ATTR_ASYNCLOADED] as $symbol) {
                self::load(BriskConfig::JS, $symbol, true);
            }
        }
    }
    ...
    private static function delAsyncDependencies($type, $symbol) {
        ...
        if (isset(self::$asyncDeleted[$type][$symbol])) {
            ...
            if ($res[BriskConfig::ATTR_DEP]) {
                foreach ($res[BriskConfig::ATTR_DEP] as $symbol) {
                    if (self::getAsync(BriskConfig::JS, $symbol)) {
                        self::delAsyncDependencies(BriskConfig::JS, $symbol);
                    }
                }
            }
            ...
            if ($res[BriskConfig::ATTR_CSS]) {
                foreach ($res[BriskConfig::ATTR_CSS] as $symbol) {
                    if (self::getAsync(BriskConfig::CSS, $symbol)) {
                        self::delAsyncDependencies(BriskConfig::CSS, $symbol);
                    }
                }
            }
